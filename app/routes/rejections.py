...
try:
    ...
    print(f"[Rejections] Data loaded successfully at {datetime.now()}")
except Exception as e:
    raise RuntimeError(f"Failed to load rejection reasons data: {e}")
...
async def rejections_search(request: Request, response: Response, query: str = Form(...)):
    ...
    print(f"[Rejections] Found {len(results)} matches for query '{query}'")
    return JSONResponse({
        "query": query,
        "results": results,
